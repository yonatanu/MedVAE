...
def main(cfg: DictConfig):
    ...
    accelerator.init_trackers("medvae", config=cfg, init_kwargs={"wandb": logger_kwargs})
    ...
    for epoch in range(cfg["max_epoch"]):
        ...
        if min_loss is None or loss < min_loss:
            try:
                accelerator.save_state(os.path.join(cfg.ckpt_dir, "best.pt"), safe_serialization=False)
        ...
        if (epoch + 1) % cfg.get("ckpt_every_n_epochs", 1) == 0:
            ...
            try:
                accelerator.save_state(os.path.join(cfg.ckpt_dir, f"epoch-{epoch:04d}.pt"), safe_serialization=False)
            except Exception as e:
                print(e)

    # save last model
    accelerator.save_state(os.path.join(cfg.ckpt_dir, "last.pt"), safe_serialization=False)
    
    # save model to output directory
    accelerator.save(model, os.path.join(cfg.output, "model.pt"), safe_serialization=False)
